...
describe("gerarPdf", () => {
  ...
  test("gera um PDF mesmo com muitas experiências e bullets longos (força redução de escala)", async () => {
    const buffer = await gerarPdf(criarCurriculoGrande());

    expect(Buffer.isBuffer(buffer)).toBe(true);
    expect(buffer.slice(0, 4).toString("latin1")).toBe("%PDF");
  });

  test("gera um PDF para um currículo com apenas parte das seções opcionais preenchidas", async () => {
    const buffer = await gerarPdf(curriculoComCamposParciais);
  ...
  test.each(curriculosComContatoVariado)(
    "gera um PDF válido para a variação de contato %#",
    async (curriculo) => {
      const buffer = await gerarPdf(curriculo);
